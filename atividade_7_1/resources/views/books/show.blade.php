...
<div class="container">
    ...
    {{-- Seção para exibir a Capa do Livro --}}
    <div class="mb-4 text-center">
    <img src="{{ asset('storage/' . $book->cover_image) }}" alt="Capa de {{ $book->title }}" class="img-fluid rounded shadow-sm" style="max-width: 250px; height: auto;">
    </div>

    {{-- CARD 1: Detalhes do Livro --}}
    <div class="card mb-4">
        ...
        <div class="card-body">
            ...
            {{-- Adicione o ano de publicação aqui se desejar --}}
            <p><strong>Ano de Publicação:</strong> {{ $book->published_year }}</p>
        </div>
    </div> {{-- FIM DO CARD DE DETALHES DO LIVRO --}}

    {{-- CARD 2: Formulário para Empréstimos --}}
    <div class="card mb-4">
        <div class="card-header">Registrar Empréstimo</div>
        <div class="card-body">
            <form action="{{ route('books.borrow', $book->id) }}" method="POST"> {{-- Use $book->id explicitamente --}}
                @csrf
                <div class="mb-3">
    ...
    </div> {{-- FIM DO CARD DE REGISTRAR EMPRÉSTIMO --}}

    {{-- CARD 3: Histórico de Empréstimos --}}
    <div class="card mb-4">
    ...
    </div> {{-- FIM DO CARD DE HISTÓRICO DE EMPRÉSTIMOS --}}
